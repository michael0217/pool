...
func (u *PayoutsProcessor) process() {
	...
	for _, login := range payees {
		...
		// Shannon^2 = Wei
		//amountInWei1 := new(big.Int).Mul(amountInShannon, util.Shannon)   // 余额乘以10^9
		amountInWei := big.NewInt(amount / 1000000000)
		//fmt.Println("取出的数据",amount," - ",amountInShannon," - ",amountInWei)
		...
		fmt.Println("余额是",poolBalance,"总额是",amountInWei)
		if err != nil {
			u.halt = true
			u.lastFail = err
			break
		...
		//value := strconv.FormatInt(amountFloat / 1000000000,10)
		//var value1 float64 = amount
		value := strconv.FormatFloat(amountFloat / 1000000000,'E',-1,64)
