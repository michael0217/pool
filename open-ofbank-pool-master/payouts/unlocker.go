...
var homesteadReward = math.MustParseBig256("913000000000000000000")
// 孤块
var byzantiumReward = math.MustParseBig256("799000000000000000000")
...
const donationAccount = "0x00007b007b404d383a7c91485759a2618bc5b2446c94536f56"
	...
	for _, candidate := range candidates {
		...
		for i := int64(minDepth * -1); i < minDepth; i++ {
			...
			if matchCandidate(block, candidate) {
				...
				//fmt.Println("原数组：",result.maturedBlocks,"新成员：",candidate)
...
func (u *BlockUnlocker) unlockPendingBlocks() {
	...
	var h = currentHeight - u.config.ImmatureDepth
	//fmt.Println("查找redis",h)
	candidates, err := u.backend.GetCandidates(h)
	if err != nil {
		u.halt = true
		u.lastFail = err
	...
	//fmt.Println("当前高度",currentHeight,"数组：",candidates)
	if len(candidates) == 0 {
...
func (u *BlockUnlocker) unlockAndCreditMiners() {
	...
	fmt.Println("当前高度",currentHeight)
