...
func NewRedisClient(cfg *Config, prefix string) *RedisClient {
	client := redis.NewClient(&redis.Options{
		Addr:     cfg.Endpoint,	// 地址
		Password: cfg.Password,	// 密码
		DB:       cfg.Database,	// 数据库
		PoolSize: cfg.PoolSize,	// 长链接的池最大数
	})
...
func (r *RedisClient) WriteBlock(login, id string, params []string, diff, roundDiff int64, height uint64, window time.Duration) (bool, error) {
	...
	if err != nil {
		...
		//fmt.Println("写入redis ",height,"--",cmd)
...
func (r *RedisClient) writeImmatureBlock(tx *redis.Multi, block *BlockData) {
	...
	//fmt.Println("删除候选区块",block.Height)
...
func (r *RedisClient) CollectStats(smallWindow time.Duration, maxBlocks, maxPayments int64) (map[string]interface{}, error) {
	...
	//te := r.client.Watch("of")
	// 创建事物块
	tx := r.client.Multi()
	defer tx.Close()  // 完成后完毕事物块

	// 时间，换算成秒
	now := util.MakeTimestamp() / 1000

	cmds, err := tx.Exec(func() error {
		// -inf 和 inf 分别表示sorted中分数的最高值和最低值
		// 缺少情况下，min和max表示的范围是闭区间范围。即min <= score <= min 内的成员将被返回。

		// ZRemRangeByScore 命令用于移除有序集中，指定分数区间内的所有成员      移除： of.hashrate 哈希率，最小值 -inf  now - window
		...
		// 返回有序集中，指定区块的成员		of:payments:all	  交易信息， (0,maxPayments-1)
